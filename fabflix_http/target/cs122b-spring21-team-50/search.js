...
function handleLookup(query, doneCallback) {
    ...
    if(suggestionsCache.has(query)){
        console.log("using CACHE to fetch suggestions");
        handleLookupAjaxSuccess(suggestionsCache.get(query), query, doneCallback);

    }else {
        // sending the HTTP GET request to the Java Servlet endpoint hero-suggestion
        // with the query data
        console.log("using AJAX to fetch suggestions");
        jQuery.ajax({
            "method": "GET",
...
function handleLookupAjaxSuccess(data, query, doneCallback) {
    ...
    suggestionsCache.set(query, data);
...
function handleSelectSuggestion(suggestion) {
    ...
    if(id){
        window.location.assign('single-movie.html?id='+ id);
    }
...
$('#title_input').keypress(function(event) {
    ...
    if (event.keyCode == 13) {
        ...
        handleNormalSearch($('#title_input').val())
    }
})
